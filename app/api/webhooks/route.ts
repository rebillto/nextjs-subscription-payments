...
export async function POST(req: Request) {
  try{
    ...
    if (userMetadata.user_metadata && userMetadata.user_metadata.hasOwnProperty("rebill_item_id")) {
      if(Array.isArray(userMetadata.user_metadata.rebill_item_id) && userMetadata.user_metadata.rebill_item_id.includes(metadataToRemove)){
        userMetadata.user_metadata.rebill_item_id = userMetadata.user_metadata.rebill_item_id.filter((item: string) => item != metadataToRemove)
        shouldUpdate = true;
      }else if(userMetadata.user_metadata.rebill_item_id == metadataToRemove){
        userMetadata.user_metadata.rebill_item_id = [];
        shouldUpdate = true;
      }else {
        return new Response(JSON.stringify({ message: 'Metadata not updated, rebill_item_id not found in user_metadata' }), {
          status: 401,
        });
      }
    }
    if(shouldUpdate){
      ...
      console.log("userMetadata: ", userMetadata.user_metadata)

      return new Response(JSON.stringify({ message: 'Metadata updated successfully' }), {
        status: 200,
      });
    }
  // Step 3: Remove the rebill_item_id from user metadata
  }
  catch (error) {
    console.error(error);
    return new Response(
